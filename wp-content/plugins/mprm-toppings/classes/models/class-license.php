...
class License extends Model {
	...
	public function activate_license() {
		...
		update_option("{$this->prefix}_status", ucfirst($licenseData->license) . __('until', 'mprm-toppings') . ' ' . $licenseData->expires);
	...
	public function get_license_status() {
		...
		switch ($licenseData->license) {
			case 'inactive':
				$message = __('Inactive', 'mprm-toppings');
				break;
			case 'site_inactive':
				$message = __('Inactive', 'mprm-toppings');
				break;
			case 'disabled' :
				$message = __('Disabled', 'mprm-toppings');
				break;
			case 'valid':
				if ($licenseData->expires !== 'lifetime') {
					...
					$message = __('Valid until', 'mprm-toppings') . $expires;
				} else {
					$message = __('Valid (Lifetime)', 'mprm-toppings');
				}
				break;

			case 'expired' :
				$message = __('Expired', 'mprm-toppings');
				break;
			case 'invalid' :
				$message = __('Invalid', 'mprm-toppings');
				break;
			case 'invalid_item_id' :
				$message = __('Product ID is not valid', 'mprm-toppings');
				break;
		}
