...
function mprm_checkout_button_purchase() {
	...
	/* $gateway = $_POST['payment-mode'];
	if($gateway == 'stripe'){
		ob_start(); ?>
		
		<input type="submit" id="mprm-purchase-stripe-button" name="mprm-purchase" value="<?php echo $complete_purchase; ?>"/>
	<?php $checkout_button = ob_get_clean();
	}else{   */
...
function mprm_checkout_delivery_tip(){
	?>
		<p id="mprm-delivery-tip">
			<label for="delivery_tip" class="mprm-label">
				<?php _e('Tip:', 'mp-restaurant-menu'); ?>
			</label>
			<input type="text" name="delivery_tip" id="delivery_tip" value="" class="medium-text" placeholder=""/>
		</p>
	<?php
}
