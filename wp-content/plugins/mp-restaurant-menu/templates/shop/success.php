...
session_start();
if($_SESSION['payment-method'] != 'stripe'){
...
//print_r($order);
    if ($status == 'Urgency'){
        ...
        $get_sub_id = get_post_meta($order->ID, 'webpushr_id'); //print_r($get_sub_id);//exit;
    	$get_subscriber_id = $get_sub_id[0];
    	$get_postmates_delivery_id = get_post_meta($order->ID, 'postmates_delivery_id');
    	...
    	$site_title = get_bloginfo( 'name' );
    			if( $get_subscriber_id !== ""){
        			$req_data = array(
        				...
        				'message' 		=> "Your order has been Placed.", //required
...
<table id="mprm_purchase_receipt">
	...
	<?php $subtotal_amount = $order->total - $order->tax; 
	if($order->gateway == "stripe"){ ?>
		<tr>
			<td><strong><?php _e('Subtotal', 'mp-restaurant-menu'); ?> </strong></td>
			<td>
				<?php echo "$ ".$subtotal_amount; ?>
			</td>
		</tr>
	<?php }else{
		if (filter_var($receipt_args['price'], FILTER_VALIDATE_BOOLEAN)) : ?>
			<tr>
				<td><strong><?php _e('Subtotal', 'mp-restaurant-menu'); ?> </strong></td>
				<td>
