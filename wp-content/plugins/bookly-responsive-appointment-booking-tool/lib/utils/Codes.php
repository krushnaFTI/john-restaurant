...
abstract class Codes
{
     ...
     * @return string
     */
    public static function replace( $text, $codes )
    {
        return self::stringify( self::tokenize( $text ), $codes );
    }
     ...
     * @return string
     */
    protected static function stringify( $tokens, $codes )
    {
        $output = '';
        ...
        foreach ( $tokens as $token ) {
            switch ( $token[0] ) {
                ...
                case 'T_IF':
                    ...
                    if ( ! empty ( $data ) ) {
                        $output .= self::stringify( $nested_tokens, $codes );
                    }
                    break;
                case 'T_EACH':
                    $data = self::get( $token[1], $codes );
                    ...
                    if ( is_array( $data) ) {
                        ...
                        foreach ( $data as $context_codes ) {
                            $parts[] = self::stringify( $nested_tokens, array( $context_code => $context_codes ) + $codes );
                        }
